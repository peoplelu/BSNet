...
class SPFormer(nn.Module):
    ...
    def loss(
        ...
        **kwargs
        ...
        sp_feats, sp_coords, sp_rgb_feats, sp_prob_labels, sp_mu_labels, sp_var_labels , loss_consistency = self.extract_feat(
            input,
            voxel_coords_float,
            voxel_rgb_feats,
            superpoints,
       ...
       # out = self.decoder(sp_feats, batch_offsets)
        ...
        # loss = loss + 0.5 * loss_consistency
    ...
    def predict(
        ...
        **kwargs
        ...
        voxel_coords_float = pointgroup_ops.voxelization(coords_float, v2p_map)
        sp_feats, sp_coords, _ , _ = self.extract_feat(input, voxel_coords_float, voxel_rgb_feats, superpoints, p2v_map)
        sp_coords_float = scatter_mean(coords_float, superpoints, dim=0)
        out = self.decoder(sp_feats, sp_coords_float, batch_offsets)
    ...
    def predict_by_feat(self, scan_ids, out, superpoints, insts, coords_float):
        # pred_labels = torch.cat([out["labels"],out["aux_outputs"][-1]["labels"]],dim=1)
        # pred_masks = [torch.cat([out["masks"][0],out["aux_outputs"][-1]["masks"][0]],dim=0)]
        # pred_scores = torch.cat([out["scores"],out["aux_outputs"][-1]["scores"]],dim=1)
        ...
        labels = (
            ...
            .unsqueeze(0)
            .repeat(pred_labels.shape[1], 1)
            .flatten(0, 1)
        )
        #print(self.test_cfg.topk_insts)
    ...
    def extract_feat(
        ...
        if self.pool == "mean":
            ...
            loss_consistency = 0
            #x = scatter_sum(scatter_softmax(self.pooling_linear((x[superpoints]-x_origin).abs()), superpoints, dim=0) * x_origin, superpoints, dim=0)
        ...
        if prob_labels is not None:
            return x, coords_float_p, rgb_feats_p, prob_labels_p, mu_labels_p, var_labels_p, loss_consistency

        return x, coords_float_p, rgb_feats_p, loss_consistency

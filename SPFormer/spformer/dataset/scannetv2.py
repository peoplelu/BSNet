...
class ScanNetDataset(Dataset):
    ...
    def transform_train(self, xyz, rgb, superpoint, semantic_label, instance_label, prob_label, mu_label, var_label, normal=None):
        ...
        mu_label = mu_label[superpoint]
        var_label = var_label[superpoint]
        ...
        #print(superpoint,mu_label.shape)
        mu_label = mu_label[valid_idxs]
        var_label = var_label[valid_idxs]
        if normal is not None:
            normal = normal[valid_idxs]
        return xyz, xyz_middle, rgb, superpoint, semantic_label, instance_label, prob_label, mu_label, var_label, normal

    def transform_test(self, xyz, rgb, superpoint, semantic_label, instance_label, prob_label, mu_label, var_label, normal=None):
        xyz_middle = xyz
        xyz = xyz_middle * self.voxel_cfg.scale
        xyz -= xyz.min(0)
    ...
    def get_instance3D(self, instance_label, semantic_label, coord_float, superpoint, scan_id):
        ...
        for i in range(num_insts):
            ...
            sem_id = semantic_label[idx].unique(return_counts=True)[0][semantic_label[idx].unique(return_counts=True)[1].argmax()]
            ...
            mean_xyz_i = coord_float_.mean(0)
            min_coord_float_ = coord_float_.min(0)[0]
            max_coord_float_ = coord_float_.max(0)[0]
            # if sem_label == -100: continue
            gt_boxes.append(torch.cat([mean_xyz_i, min_coord_float_, max_coord_float_], dim=0))
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        xyz, xyz_middle, rgb, superpoint, semantic_label, instance_label, prob_label, mu_label, var_label, normal = (
            self.transform_train(xyz, rgb, superpoint, semantic_label, instance_label, prob_label, mu_label, var_label, normal)
            if self.training
            else self.transform_test(
                xyz, rgb, superpoint, semantic_label, instance_label, prob_label, mu_label, var_label, normal
            )
        )

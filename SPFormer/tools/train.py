...
def get_args():
    ...
    parser.add_argument("--exp_name", type=str, default="DTAS")
...
def main():
    ...
    if args.resume:
        ...
        checkpoint_sst = torch.load('/ssd/ljh/3d_ins/GaPro/SPFormer/checkpoints/sstnet_pretrain.pth')
        state_dict_sst = checkpoint_sst["model"]
        for key in state_dict.keys():
            if key.startswith("unet."):
                if state_dict[key].shape != state_dict_sst[key].shape:
                    state_dict[key] = state_dict[key].permute(-1,0,1,2,3)
            if key.startswith("input_conv."):
                if state_dict[key].shape != state_dict_sst[key].shape:
                    state_dict[key] = state_dict[key].permute(-1,0,1,2,3)

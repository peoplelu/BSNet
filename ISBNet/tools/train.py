...
def validate(epoch, model, optimizer, val_loader, cfg, logger, writer):
    ...
    point_eval = PointWiseEval(num_classes=cfg.model.instance_classes + 2)
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            # if cfg.data.train.type == "s3dis" and batch["coords_float"].shape[0] > 2000000:
            #     print(batch["scan_ids"])#Area_5_hallway_15 Area_5_hallway_4 Area_5_office_20 Area_5_office_36 Area_5_office_3
            #     continue
            ...
            if cfg.data.train.type == "scannetv2":
                ...
                xyz = batch['coords_float'].cpu().numpy().astype(np.float)
                semantic_label = batch['semantic_labels'].cpu().numpy().astype(np.int32)
                instance_label = batch['instance_labels'].cpu().numpy().astype(np.int32)

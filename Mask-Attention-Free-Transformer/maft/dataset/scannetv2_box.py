...
class ScanNetDataset(Dataset):
    ...
    def load(self, filename):
        if self.with_normals:
            ...
            normal = None
        if self.with_label:
            xyz, rgb, dummy_sem_label, dummy_inst_label = torch.load(filename)
            scene_id = filename.split('/')[-1][:12]
            
            ps_filename = osp.join(self.data_root.replace('scannetV2','Gapro'), 'cross_scene4', scene_id + ".pth")
            if self.prefix == 'train':
                dummy_sem_label, dummy_inst_label, _, _, _ = torch.load(
                    ps_filename
                )
                
            superpoint = torch.load('/ssd/dataset/ScanNetV2_seg/Gapro/superpoints/'+scene_id+'.pth')
    ...
    def get_instance3D(self, instance_label, semantic_label, superpoint, coord_float, scan_id):
        ...
        #inst.semantic_label = semantic_label
        
        #inst.bbox_label = bbox_label
        return inst
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        if semantic_label is not None:
            ...
            if self.prefix == 'train':
                semantic_label[semantic_label==18] = -100
            else:
